...
class Vouchers extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($count > 0) {
            ...
            foreach ($_data as $d) {
				...
				$produk_bonus_name = '';
				if($d->tipe == 3){
					$produk_utama_name = $_dp[$d->produk_utama];
					$produk_bonus_name = $_dp[$d->produk_bonus];
				}
	...
	function get_voucher_available(Request $request){
		...
		$sql_list_product = "select id_product, id_voucher from list_produk_voucher where id_product in (" . $_whereIn . ") and deleted_at is null";
